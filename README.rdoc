...
utilitário Java::
  So you have lost your class? Find it back with: <code>java -jar whereis.jar[http://cua.li/TI/whereis.jar] MyClass /in/MyApps</code>
  
  As this tool is making use of the temporary directory to expand the embedded archives, you may have to change the <code>java.io.tmpdir</code> property to a directory you have write permissions to.
  * <code>java -Djava.io.tmpdir=D:\tmp -jar whereis.jar[http://cua.li/TI/whereis.jar] SAXParserFactory D:\JBoss</code>
  
  More explanations on {cua.li/TI}[http://cua.li/TI/whereIsMyClass.html]'s site

componentes Visage (ex-JavaFX Script)::
