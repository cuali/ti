...
class HexagonalColorPicker extends Pane {
    ...
    */
    ...
    def chosen :Color = {
        chosenProperty.value.asInstanceOf[Color]
    }
    private[this] val chosenProperty :ObjectProperty[Paint] = new ObjectProperty(Color.TRANSPARENT, "chosenProperty")
    /*
    chosenProperty onChange {
        chosen = chosenProperty.value.asInstanceOf[Color]
    }
    private[this] def chosen_=(paint :Color) {
        chosen = paint
    }
    */
    ...
    private[this] val radiusProperty = new DoubleProperty(Double.box(100), "radius")
    ...
    visible onChange {
        (_, oldVisible, newVisible) =>
	        if (newVisible) {
                this.managed = true
	            chosenProperty.value = original
	            this.toFront()
	        }
    } 
    managed = false
    private[this] val centralLabel = new StringProperty("", "centralLabel")
    private[this] val centralPaint :ObjectProperty[Paint] = new ObjectProperty(Color.TRANSPARENT, "centralPaint")
    private[this] var centralHexagon :RegularPolygon = new RegularPolygon { sides = 10 }
    private[this] var currentHexagon :RegularPolygon = new RegularPolygon { angle = 30.0 }
    /**
        ...
        for (cell <- HexagonalColorPicker.CELLS) {
            ...
            val hexagon = new RegularPolygon {
                ...
                onMouseEntered = (me :MouseEvent) => {
                    ...
                    val node = me.getSource().asInstanceOf[RegularPolygon]
                    node.toFront()
                    node.transforms += Transform.scale(2, 2, node.centerX, node.centerY)
                    currentHexagon = node
                    colorName.text = cellColor
                ...
                onMouseExited = (me :MouseEvent) => {
                    colorName.visible = false
                    me.getSource().asInstanceOf[RegularPolygon].transforms.clear
                }
                onMouseClicked = (me :MouseEvent) => {
                    centralLabel.value = cellColor
                    centralPaint.value = colorName.fill.value
                    centralHexagon.fill <== (me.getSource().asInstanceOf[RegularPolygon]).fill
                }
            }
            content += hexagon
        }
