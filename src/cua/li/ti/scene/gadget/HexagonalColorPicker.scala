...
class HexagonalColorPicker extends Pane {
    ...
    */
    ...
    def chosen :Color = {
        chosenProperty.value.asInstanceOf[jfxsp.Color]
    }
    private[this] val chosenProperty :ObjectProperty[jfxsp.Paint] = new ObjectProperty(Color.TRANSPARENT, "chosenProperty")
    ...
    def computePrefWidth = 2 * radiusProperty.value
    def computePrefHeight = HexagonalColorPicker.SQRT_3 * radiusProperty.value
    /**
    ...
    def layoutChildren {
        content = Seq(
            new RegularPolygon {
                ...
                radius = HexagonalColorPicker.this.radius.value
                angle = 0
        ...
        for (cell <- HexagonalColorPicker.CELLS) {
            ...
            hexagon.onMouseClicked = (me :MouseEvent) => {
                ...
                centralPaint = colorName.fill.value.asInstanceOf[jfxsp.Color]
                centralHexagon.fill = hexagon.fill.value
            }
            content += hexagon
        }
    }
